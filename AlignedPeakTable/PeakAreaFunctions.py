...
def get_areas_df(file_path_input):
    ...
    areas_df = GeneralFunctions.get_csv(return_type='df', title='Areas', initialdir=file_path_input, skip_blank_lines=True, skiprows=1)
...
def process_areas(file_path_input, file_path_output, profile_df):
    # get_areas = ask_areas()
    get_areas = True
    if get_areas is True:
        ...
        output_file_name_path1 = f'{file_path_output}/{output_file_name1}'
        output_file_name_path2 = f'{file_path_output}/{output_file_name2}'
        try:
            detailed_areas_df.to_csv(output_file_name_path1, sep=',', na_rep='', index=False, header=False)
            print(f"{output_file_name1} export complete")
        except:
        ...
        try:
            ...
            print(f"{output_file_name2} export complete")
        except:
